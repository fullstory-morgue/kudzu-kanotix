...
int main(int argc, char **argv)
{
	...
	/* Detailed timing information. */
		...
		struct edid_detailed_timing *timing = NULL;
		...
		if(monitor->type == edid_monitor_descriptor_ascii) {
			printf("Monitor details %d:\n", i);
			printf("\tASCII String: %s:\n",
			       snip(monitor->data.string));
		} else
		if(monitor->type == edid_monitor_descriptor_name) {
			printf("Monitor details %d:\n", i);
			printf("\tName: %s\n",
			       snip(monitor->data.string));
		} else
