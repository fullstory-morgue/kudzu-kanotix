	...
	if (
		...
		while ((ent = readdir(dir))) {
			...
			while ((scsient = readdir(scsidir))) {
				if (!strncmp(scsient->d_name,"block:",6)) {
					sdev->device = strdup(scsient->d_name+6);
				} else if (!strncmp(scsient->d_name,"block",5)) {
				        char *t = canonicalize_file_name(scsient->d_name);
					if (t) {
						sdev->device = strdup(basename(t));
						free(t);
					}
				}
				if (!strncmp(scsient->d_name,"tape:",5)) {
					sdev->device = strdup(scsient->d_name+5);
				} else if (!strncmp(scsient->d_name,"tape",4)) {
				        char *t = canonicalize_file_name(scsient->d_name);
					if (t) {
						sdev->device = strdup(basename(t));
						free(t);
				...
				if (!sdev->device) {
					if (!strncmp(scsient->d_name,"generic:",8)) {
						sdev->device = strdup(scsient->d_name+8);
					} else if (!strncmp(scsient->d_name,"generic",7)) {
						char *t = canonicalize_file_name(scsient->d_name);
						if (t) {
							sdev->device = strdup(basename(t));
							free(t);
